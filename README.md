...
// LÓGICA CORREGIDA A TU HARDWARE (ACTIVE-HIGH)
#define RELE_ENCENDIDO HIGH
#define RELE_APAGADO LOW
...
// 5. MÁQUINA DE ESTADOS Y TIEMPOS (FSM)
// ==========================================================
enum EstadoSistema : uint8_t { CALENTANDO, MONITOREANDO, ALERTA_RUIDO, ALERTA_CO2 };
EstadoSistema estadoActual = CALENTANDO;

const uint32_t TIEMPO_CALENTAMIENTO_MS = 240000; 
const uint32_t TIEMPO_ACTIVO_MS = 180000;        
const uint64_t TIEMPO_DORMIR_US = 3180000000ULL; 
...
void loop() {
  ...
  // --------------------------------------------------------
    ...
    if (ahora - tiempoAnteriorMuestreo >= 500) {
      ...
      // SE ELIMINÓ EL DISPARADOR DE ERROR SEGURO PARA EVITAR FALSOS POSITIVOS POR EL WI-FI

      ruidoActual = map(amplitudSonido, 0, 4095, 30, 500);
    ...
    if (ahora - tiempoAnteriorPantalla >= 1000) {
      ...
      // MITAD SUPERIOR: RUIDO
      display.setTextSize(1); 
      display.setCursor(0, 0);
      display.print(F("RUIDO:"));
      display.setTextSize(2); 
      display.setCursor(0, 10);
      display.print(ruidoActual); 
      display.println(F(" dB"));

      // MITAD INFERIOR: CO2
      display.setTextSize(1); 
      display.setCursor(0, 32);
      display.print(F("AIRE (CO2):"));
      display.setTextSize(2); 
      display.setCursor(0, 42);
      display.print(aireActual, 0); // El ", 0" le quita los decimales para que se vea más limpio
